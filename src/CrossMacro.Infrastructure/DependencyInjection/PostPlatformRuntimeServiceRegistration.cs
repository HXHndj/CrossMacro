...
internal static class PostPlatformRuntimeServiceRegistration
{
    internal static void Register(IServiceCollection services)
    {
        ...
        _ = services.AddSingleton<IRunExecutionService>(sp => new RunScriptRuntimeService(sp.GetRequiredService<Func<IMacroPlayer>>(), sp.GetRequiredService<IKeyCodeMapper>(), sp.GetService<IMousePositionProvider>(), coarseDelayStrategy: sp.GetService<ICoarseDelayStrategy>()));
        ...
        if (services.Any(descriptor => descriptor.ServiceType == typeof(InputCaptureSessionFactory))
            && services.All(descriptor => descriptor.ServiceType != typeof(InputCaptureSessionCoordinator)))
        {
